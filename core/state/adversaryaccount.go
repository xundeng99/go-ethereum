...
type AccountIdentityHelper struct {
	addr common.Address
	id PotentialIdentity	
}
...
func (aa *AdversaryAccount) set_token_flow(addrfrom common.Address, addrto common.Address, amt common.Hash, token_addr common.Address) {
	if _, ok := ERC_TOKEN_INFORMATION_MAP[token_addr]; ok {
		...
		//in the case of nft amt is the token id
	}
}
...
func (aa *AdversaryAccount) token_transfer_nft_check(assemble bool) bool {
	potential_identity_list := make(map[common.Address]PotentialIdentity)
	owner_initial := make(map[common.Address]map[common.Hash]common.Address)
	owner_current := make(map[common.Address]map[common.Hash]common.Address)
	...
	for _, i := range aa.transfer_in_order {
		...
		amt := i.amt
		token_addr := i.token
		for _, token := range NFT_CONTRACT_ADDRESSES {
			//fmt.Println("Transfering NFT ", token_addr, " ID ", amt, " from ", from, "to ", to)
			if token_addr == token {
				if _, ok := owner_initial[token_addr]; ok{

				}else{
					// initial owner of the nft
					owner_initial[token_addr] = make(map[common.Hash]common.Address)
					owner_initial[token_addr][amt] = from					
				}
				if _, ok := owner_current[token_addr]; ok{
					owner_initial[token_addr][amt] = to
				}else{
					owner_initial[token_addr] = make(map[common.Hash]common.Address)
					owner_initial[token_addr][amt] = to
				}
				if _, ok := potential_identity_list[from]; ok{
					if potential_identity_list[from] == Beneficiary {
						potential_identity_list[from] = Neither
					}

				} else {
					potential_identity_list[from] = Victim;
				}
				if _, ok := potential_identity_list[to]; ok{
					if potential_identity_list[to] == Victim {
						potential_identity_list[from] = Neither
					}

				} else {
					potential_identity_list[to] = Beneficiary;
				}
			}
		}
	}
	...
	beneficiary_ERC20 := make(map[common.Address]PotentialIdentity)
	for addr, result := range aa.flash_loan_information {
		if result.id == Beneficiary {
			beneficiary_ERC20[addr] = Beneficiary
			
		} else if result.id == Victim {
			beneficiary_ERC20[addr] = Victim
		}
	}
	var beneficiary_list []common.Address
	var victim_list []common.Address
	for addr, id := range(potential_identity_list){
		if id == Beneficiary{
			if id_erc20, ok := beneficiary_ERC20[addr]; ok{
				if id_erc20 == Beneficiary {
					beneficiary_list = append(beneficiary_list, addr)					
				}
			}else {
				beneficiary_list = append(beneficiary_list, addr)
			}
			...
			if id_erc20, ok := beneficiary_ERC20[addr]; ok{
				if id_erc20 == Victim{
					victim_list = append(victim_list, addr)				
				}
			} else{
				victim_list = append(victim_list, addr)
			}
	...
	for _, i := range beneficiary_list {
		fmt.Println("Beneficiary: ", i)
		if i == aa.old_tx.From() || i == *(aa.old_tx_contract_address) {
			found_in_beneficiary = true
		}
	}
	for _, i := range victim_list {
		fmt.Println("Vicitm: ", i)
	}
	...
	msg := types.NewMessage(FRONTRUN_ADDRESS, aa.old_tx.To(), aa.old_tx.Nonce(), aa.old_tx.Value(), aa.old_tx.Gas(), aa.old_tx.GasPrice(), aa.old_tx.GasFeeCap(), aa.old_tx.GasTipCap(), new_data, aa.old_tx.AccessList(), true)
	aa.new_tx = &msg

	return found_in_beneficiary
...
func (aa *AdversaryAccount) assemable_new_transactions() {
	if aa.old_tx.To() == nil {
		...
		if deploy_gas_price, deploy_gas, deploy_gas_fee_cap, deploy_gas_tip_cap, deploy_value, is_create_action, call_address, deploy_data, ok := Get_contract_init_data_with_init_call(*(aa.old_tx_contract_address)); ok == nil {
			...
			if len(aa.target_beneficiary_addresses) != 0 {
				fmt.Println("replace beneficiary")
				for _, addr_to_be_replaced := range aa.target_beneficiary_addresses {
			...
			fmt.Println("call addr, value: ", tmp_call, call_data)
